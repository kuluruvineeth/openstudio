...
import { VisitorStats } from "@/app/api/youtube/analytics/controller";
import {
  BarChart,
  BarChartEventProps,
...
export const VisitorAnalytics = (props: {
  ...
  const { data, isLoading, error } = useQuery<VisitorStats[]>({
    queryKey: ["visitorAnalytics", props.query],
    queryFn: async () =>
      (await useChannelAnalytics(props.query)) as unknown as VisitorStats[],
    refetchOnWindowFocus: false,
    // refetchInterval: props.refreshInterval, //TODO: Need to uncomment after development phase, not needed for now
  });

  const [processedData, setProcessedData] = useState(() =>
    aggregateData(data ?? [], groupByPeriod, ["views"]),
  );
  // Update processedData whenever data or groupByPeriod changes
  useEffect(() => {
    setProcessedData(aggregateData(data ?? [], groupByPeriod, ["views"]));
  }, [data, groupByPeriod]);
      ...
      <Card>
        <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
          <div className="grid flex-1 gap-1 text-center sm:text-left">
            <CardTitle>{t("VISITOR.TITLE")}</CardTitle>
        ...
        <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
          <BarChart
            ...
            postiveFillColor="fill-blue-600 dark:fill-blue-500"
          />
        </CardContent>
      </Card>
    </LoadingContent>
