...
import { transcriptionItemsToSrt } from "@/lib/transciption-helper";
...
export default function ResultVideo({ filename, transcriptionItems }: any) {
  const videoUrl = "https://dv3gl816rsprk.cloudfront.net/" + `${filename}`;
  ...
  const transcode = async () => {
    ...
    ffmpeg.on("log", ({ message }) => {
      ...
      if (regexResult && regexResult?.[1]) {
        ...
        const [hours, minutes, seconds]: any[] = howMuchIsDone.split(":");
    ...
    await ffmpeg.exec([
      ...
      "ultrafast",
      "-vf",
      `subtitles=subs.srt:fontsdir=/tmp:force_style='Fontname=Roboto Bold,FontSize=20,MarginV=30,PrimaryColour=${toFFmpegColor(
        primaryColor
